...
setTimeout(function() {FetchData();}, 8000);
function FetchData() {
  ...
  console.log("HAPPY: ");
  for (const so of happy_songs){
    process.stdout.write("spotify:track:" + so + ",");
  }

  console.log("SAD: ");
  for (const so of sad_songs){
    process.stdout.write("spotify:track:" + so + ",");
  }
  console.log("");
  console.log("DANCE: ");
  for (const so of dance_songs){
    process.stdout.write("spotify:track:" + so + ",");
  }

  console.log("");
  console.log("CHILL: ");
  for (const so of chill_songs){
    process.stdout.write("spotify:track:" + so + ",");
  }
}
